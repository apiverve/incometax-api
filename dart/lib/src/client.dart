...
class IncometaxValidationException implements Exception {
  final List<String> errors;

  IncometaxValidationException(this.errors);

  @override
  String toString() => 'IncometaxValidationException: ${errors.join("; ")}';
}
...
/// Income Tax Brackets API client.
///
/// For more information, visit: https://apiverve.com/marketplace/incometax?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [country] (required) - The country code (currently only 'US' is supported) [minLength: 2, maxLength: 2]
/// * [year] - The tax year to retrieve brackets for. Defaults to the previous year. [min: 2000, max: 2030]
class IncometaxClient {
  ...
  IncometaxClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/incometax',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [IncometaxValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw IncometaxValidationException(errors);
    }
  }

  /// Execute a request to the Income Tax Brackets API.
  ///
  /// Parameters are validated before sending the request.
  Future<IncometaxResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw IncometaxException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return IncometaxResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw IncometaxException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw IncometaxException('Resource not found');
      } else {
        throw IncometaxException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is IncometaxException) rethrow;
      throw IncometaxException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Income Tax Brackets API client.
class IncometaxException implements Exception {
  final String message;

  IncometaxException(this.message);

  @override
  String toString() => 'IncometaxException: $message';
}
